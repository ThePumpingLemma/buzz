...
fn otel_env_filter(rust_log: Option<&str>) -> EnvFilter {
    EnvFilter::new(rust_log.unwrap_or("buzz_relay=info,buzz_datastore=info"))
}
use uuid::Uuid;
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        ...
        .with(otel_layer.map(|layer| {
            layer.with_filter(otel_env_filter(std::env::var("RUST_LOG").ok().as_deref()))
        }))
        .init();
...
mod env_filter_tests {
    use super::{log_env_filter, otel_env_filter};
    ...
    #[test]
    fn explicit_global_warn_is_preserved_alone() {
        assert_eq!(log_env_filter(Some("warn")).to_string(), "warn");
        assert_eq!(otel_env_filter(Some("warn")).to_string(), "warn");
    }
}
